...
exports.getBalances = async (req, res, next) => {
  try {
    ...
    const transactions = await Transaction.find({ group: req.params.id, status: 'completed' });
    const group = await Group.findById(req.params.id).populate('members', 'id name upiId');

    if (!group) return res.status(404).json({ error: 'Group not found' });

    let balancesMap = {};
    group.members.forEach(m => { 
        balancesMap[m._id.toString()] = { 
            id: m._id, 
            name: m.name, 
            upiId: m.upiId, 
            balance: 0 
    ...
    transactions.forEach(t => {
        const payerId = t.payer.toString();
        const payeeId = t.payee.toString();
        if (balancesMap[payerId]) balancesMap[payerId].balance += t.amount;
        if (balancesMap[payeeId]) balancesMap[payeeId].balance -= t.amount;
    });
